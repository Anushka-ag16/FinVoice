...
export default function PortfolioAnalyzerPage() {
  ...
  const getCorrelationColor = (val: number) => {
    if (val === 1) return "bg-slate-100 text-slate-400";
    ...
    <div className="flex flex-col gap-8 pb-20 md:pb-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 font-medium">Last updated: Today, 09:41 AM</p>
        </div>
        <button className="px-4 py-2 rounded-xl bg-white border border-slate-200 text-slate-700 font-medium hover:bg-slate-50 transition shadow-sm">
          Export Report
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">Portfolio Beta vs NIFTY50</span>
        ...
        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">Sharpe Ratio (1Y)</span>
          <div className="flex items-end gap-3 mt-4">
            <span className="text-4xl font-bold font-mono text-slate-900">0.91</span>
            <span className="px-2 py-1 bg-blue-500/20 text-blue-600 text-xs rounded mb-1">Good</span>
          </div>
          <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between text-xs text-slate-400">
        ...
        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">Max Drawdown (1Y)</span>
          <div className="flex items-end gap-3 mt-4">
          ...
          <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between text-xs text-slate-400">
            ...
            <span className="text-slate-500">Recovered in 42 days</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm flex flex-col">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-bold text-slate-900">Concentration Risk</h2>
            <span className="text-xs text-slate-400 uppercase tracking-widest">Top 5 Holdings</span>
          </div>
            ...
            {[
              ...
              <div key={st.name} className="flex flex-col gap-1">
                <div className="flex justify-between text-sm font-medium">
                  ...
                  <span className={cn("font-mono", st.val > st.limit ? "text-danger-accent" : "text-slate-500")}>{st.val}%</span>
                </div>
        ...
        <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-bold text-slate-900">Sector Exposure</h2>
          ...
          <div className="h-8 w-full rounded-lg overflow-hidden flex mb-8 border border-slate-200">
            <div className="h-full bg-blue-500 transition-all hover:opacity-80 cursor-pointer" style={{ width: '38%' }} title="IT / Tech: 38%" />
            <div className="h-full bg-slate-400 transition-all hover:opacity-80 cursor-pointer" style={{ width: '25%' }} title="Financials: 25%" />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 border border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-slate-700">IT / Tech</span>
              </div>
            ...
            <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 border border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-slate-700">Financials</span>
              </div>
              <span className="font-mono text-sm text-slate-600">25.0%</span>
            </div>
            <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 border border-slate-100">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded bg-emerald-500" />
                <span className="text-sm font-medium text-slate-700">FMCG</span>
              </div>
              <span className="font-mono text-sm text-slate-600">15.0%</span>
            </div>
            <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 border border-slate-100">
              <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-sm text-slate-600">10.0%</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm overflow-x-auto">
        <h2 className="text-lg font-bold text-slate-900 mb-2">Correlation Heatmap</h2>
        <p className="text-sm text-slate-500 mb-6">Highly correlated pairs (≥ 0.75) may reduce diversification benefits.</p>

        <div className="min-w-[600px]">
          <div className="grid grid-cols-[100px_repeat(5,1fr)] gap-1 mb-1">
            <div />
            {stocks.map(s => <div key={s} className="text-xs font-bold text-slate-400 text-center py-2">{s}</div>)}
          </div>
          {correlations.map((row, i) => (
            <div key={`row-${i}`} className="grid grid-cols-[100px_repeat(5,1fr)] gap-1 mb-1">
              <div className="text-xs font-bold text-slate-400 flex items-center">{stocks[i]}</div>
              {row.map((val, j) => (
                <div 
                  key={`cell-${i}-${j}`} 
                  className={cn(
      ...
      <div className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-200 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
          <div>
             <h2 className="text-lg font-bold text-slate-900">Rupee Rebalancing Plan</h2>
             <p className="text-xs text-slate-500 mt-1">AI-optimized trades to correct drift and minimize taxes.</p>
          </div>
          <button className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-medium transition shrink-0 flex items-center justify-center gap-2">
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100">
              <tr className="bg-white hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4 text-right font-mono text-slate-700">8,200</td>
                <td className="px-6 py-4">
              ...
              <tr className="bg-slate-50 hover:bg-slate-100 transition-colors">
                ...
                <td className="px-6 py-4 font-medium text-slate-900">SGB 2024-25</td>
                <td className="px-6 py-4 text-right font-mono text-slate-700">5,000</td>
                <td className="px-6 py-4">
              ...
              <tr className="bg-white hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4 font-medium text-slate-900">PPFAS Flexi Cap</td>
                <td className="px-6 py-4 text-right font-mono text-slate-700">3,200</td>
                <td className="px-6 py-4">
                   <span className="text-slate-400 text-xs">—</span>
                </td>
              </tr>
